...
public:
    ...
    int totalObject = 1;
    bool anisoShade = false;
    bool toonShade = false;
    bool coneShape = false ;
...
void display() {
    ...
    for (int y = -objDrawRange; y <= objDrawRange; y++) {
        for (int x = -objDrawRange; x <= objDrawRange; x++) {
            vec3 pos;
            if(viewport.coneShape)
                pos = customShape(x * iObjDrawRange, y * iObjDrawRange);
            else
                pos = circleShape(x * iObjDrawRange, y * iObjDrawRange);
            ...
            for(int k = -maxProjDist; k <= maxProjDist && drawnObject < viewport.totalObject; k += 2) {
                for(int l = -maxProjDist; l <= maxProjDist && drawnObject++ < viewport.totalObject; l += 2) {
                    vec3 movObjPos(k, l, 0.0f);
                    vec3 movObjPixPos = movObjPos * objDrawRange;
                    vec3 col;
                    if(viewport.anisoShade)
...
///////////////////////////////////////////////////////////////////////////////////////////////////
//****************************************************
// compute the pixel map vector[y position][x position]
//****************************************************
...
void parseArguments(int argc, char* argv[]) {
    ...
    while (i < argc) {
        if (strcmp(argv[i], "-ka") == 0) {
            ...
            // MultipleObject
            cout << "Number of Object(s): ";
            viewport.totalObject = max(atoi(argv[i + 1]), 1);
            cout << viewport.totalObject << endl;
            i += 2;
        } else if(strcmp(argv[i], "-co") == 0) {
            // Cone Shape
            cout << "Cone Shape" << endl;
            viewport.coneShape = true;
            i ++;
        } else if(strcmp(argv[i], "-an") == 0) {
            // Anisotropic Shading
            cout << "Anisotropic Shading" << endl;
            viewport.anisoShade = true;
            i ++;
        }
